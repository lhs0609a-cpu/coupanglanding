...
> **핵심 원리**: 누끼 PNG의 투명영역을 마스크로 삼아 **배경만 새로 생성**하고
> 상품 픽셀은 그대로 보존합니다(SDXL 인페인트). 쿠팡 대표이미지는 "실제 상품과 동일"
> 해야 하므로, 상품을 통째로 새로 그리는 방식(IP-Adapter 등)은 정책·오인 위험이 있어
> 기본 워크플로는 **상품 보존형 인페인트**입니다.
