  ...
  await pool.withWindow('list', async (sw) => {
    ...
    for (let i = 1; i <= maxScrolls; i++) {
      ...
      if (gained === 0) {
        ...
        if (noNew >= limit) {
          // ⚠️ 예전에 여기서 "카테고리 화면은 약 60개짜리 큐레이션" 이라고 안내했는데
          //    **그건 틀린 설명이었다**(실측 2026-08-25~26으로 반증). 같은 카테고리에서
          //    크롬 + 진짜 입력으로는 244개까지 나왔다. 이 창이 50개에서 멈추는 진짜 이유는
          //    이 경로가 합성 이벤트(isTrusted=false)를 쓰고 진짜 휠도 아니라서다.
          //    → 크롬 경로(collect-list-chrome.mjs)가 정답이고, 여기는 폴백일 뿐이다.
          stopped = items.size < target * 0.8
            ? `이 창으로는 ${items.size}개에서 멈춥니다 — 크롬 수집기를 쓰면 더 나옵니다(같은 카테고리 244개 실측)`
            : '더 나올 것이 없음';
          return;
        }
      } else {
