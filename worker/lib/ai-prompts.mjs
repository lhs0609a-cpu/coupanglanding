...
[상품명 구조] 아래 순서로 띄어쓰기로만 나열한다(문장 아님):
  제품 핵심명 → 핵심스펙(용량/수량/규격/맛/종류/재질) → 검색 속성 키워드 3~5개(용도·대상·형태·특성 등)
[규칙]
- ⚠️ 브랜드명은 절대 넣지 말 것. 브랜드는 별도 필드로 등록되므로 노출명에서는 뺀다(예: "현대농산 발아현미"→"발아현미", "에코서트 알로에"→"알로에"). 상호·상표·브랜드로 보이는 고유명(회사명·스토어명)은 제외한다.
- 길이: 공백 포함 50~70자(최대 90자). ⚠️ 50자보다 짧으면 안 된다 — 실제 검색되는 속성어를 풍부하게 붙여 길고 다양하게.
...
- 검색 속성어를 다양하게: 용도(요리용·선물용·간식용), 대상(성인·아이·반려), 형태(대용량·개별포장·낱개), 동의어(다진마늘=깐마늘) 등 서로 다른 각도의 명사를 3~5개 덧붙인다.
...
출력은 JSON만: {"displayName": "...", "keywords": ["...8개..."]}. keywords 는 상품명에 다 못 넣은 추가 검색어(동의어·용도·대상) 8개.
⚠️ keywords 는 상품명에 이미 쓴 단어와 겹치지 않게 서로 다른 각도로 8개를 채운다 — 노출명이 짧게 나왔을 때
   이 목록으로 길이를 보정하므로, 겹치는 단어만 주면 상품명이 짧은 채로 등록된다.`;
...
- 원본 "곰곰 국내산 깐마늘 1kg 아삭한 요리용" → {"displayName":"깐마늘 1kg 다진마늘 요리용 김장 반찬 대용량 아삭한 마늘","keywords":["깐마늘","다진마늘","김장마늘","요리용마늘","마늘1kg"]}  ← 원본의 "국내산"은 뺀다
- 원본 "현대농산 국산 발아현미 20곡 혼합곡 2kg" → {"displayName":"발아현미 20곡 혼합곡 2kg 잡곡밥 건강 잡곡 대용량 요리용","keywords":["발아현미","혼합곡","잡곡","20곡","현미밥"]}  ← 브랜드와 "국산"을 함께 뺀다
[나쁜 예(문장 원본) → 이렇게 명사구로 고쳐라]
- 원본 "에서 정성스럽게 재배한 기능성 쌀 입니다 안심하고 드셔도 됩니다 혼합곡 18곡 4kg" (설명 문장)
  → (X) "에서 정성스럽게 재배한 기능성 쌀 입니다 …"  (O) {"displayName":"기능성 쌀 혼합곡 18곡 4kg 잡곡밥 건강잡곡 대용량 요리용 반찬용","keywords":["혼합곡","18곡잡곡","기능성쌀","잡곡밥","대용량잡곡"]}
→ 위 상품의 노출상품명(displayName)과 검색 키워드 8개를 JSON으로. 브랜드명 없이, 제품명+스펙+다양한 속성 검색어로 길고(50자+) 풍부하게. 명사만 나열, 문장·반복 금지.${fixNote ? `\n\n[직전 출력 문제 — 반드시 교정] ${fixNote}` : ''}`;
