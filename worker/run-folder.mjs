...
async function main() {
   ...
   *   1콜에 88.0초(같은 PC GPU 15.0초). 실제 판정은 24칸 격자(1200x1800급)에 출력도 더 길고
   *   상품당 2콜(대표+리뷰)을 동시에 던지므로 무GPU PC 는 90초 상한을 넘기기 쉽다.
   *   품질은 그대로다: 상한 초과 시의 결과가 원래 CLIP 폴백이라 결과물이 달라지지 않는다.
   *
   *   ⚠️ 2 로 충분한 이유(콜드스타트 오판 걱정 없음) — **모델 적재(5.6GB)는 첫 호출에만** 든다.
   *      ensureModel 은 설치 여부만 보고 메모리에 올리지 않으므로 첫 상품이 적재를 떠안지만,
   *      generateVision 의 keep_alive 가 30분이라 이후 상품은 이미 적재된 모델을 쓴다.
   *      즉 2번째 상품까지 상한을 넘겼다면 그건 적재 탓이 아니라 **이 PC 의 정상 속도**다.
   *      (3 으로 늘리면 느린 PC 에서 90초를 한 번 더 버릴 뿐 얻는 게 없다.)
   */
