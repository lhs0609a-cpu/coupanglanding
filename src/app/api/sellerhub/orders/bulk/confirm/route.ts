...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const order of orders) {
      const key = `${(order as Record<string, unknown>).sellerhub_user_id}:${(order as Record<string, unknown>).channel}`;
    ...
    for (const [, channelOrders] of grouped) {
      ...
      try {
        const adapter = await getAuthenticatedAdapter(serviceClient, first.sellerhub_user_id as string, first.channel as Channel);
