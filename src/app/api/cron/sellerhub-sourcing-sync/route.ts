...
import { AliexpressAdapter } from '@/lib/sellerhub/adapters/aliexpress.adapter';
import { Ali1688Adapter } from '@/lib/sellerhub/adapters/ali1688.adapter';
...
export async function GET(request: Request) {
  ...
  for (const product of products) {
    ...
    const shUserId = p.sellerhub_user_id as string;
      ...
      if (!isAvailable) {
        ...
        if (linked) {
          for (const prod of linked) {
            ...
            // 각 채널에서 품절 처리
            try {
              const adapters = await getAllAuthenticatedAdapters(supabase, prodData.sellerhub_user_id as string);
