...
import { getAllAuthenticatedAdapters } from '@/lib/sellerhub/adapters/factory';
import { CHANNEL_ORDER_STATUS_MAP } from '@/lib/sellerhub/constants';
import type { Channel, OrderStatus } from '@/lib/sellerhub/types';
          ...
          for (const item of result.items) {
            ...
            await supabase
              ...
              .upsert({
                ...
                buyer_name: String(item.buyerName || item.orderer?.name || ''),
                receiver_name: String(item.receiverName || item.receiver?.name || ''),
                receiver_phone: String(item.receiverPhone || item.receiver?.tel1 || ''),
                receiver_address: String(item.receiverAddress || item.receiver?.addr1 || ''),
