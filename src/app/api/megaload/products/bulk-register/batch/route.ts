    ...
    async function registerSingleProduct(product: BatchProduct): Promise<ProductResult> {
      ...
      try {
        ...
        if (isNoticeError) {
          ...
          console.error(`[batch][v3] 고시정보 에러 감지 — notices 제거 후 재시도: ${product.productCode}, itemKeys=${Object.keys((payload.items as Record<string,unknown>[])?.[0] || {}).join(',')}`);
          ...
          try {
            ...
            const retryMsg = `[v3] ${retryErr instanceof Error ? retryErr.message : '쿠팡 API 등록 실패 (재시도)'}`;
            return {
              uid: product.uid, productCode: product.productCode, name: product.name,
          ...
          return {
            uid: product.uid, productCode: product.productCode, name: product.name,
            success: false, error: `[v3] ${errMsg}`, duration: Date.now() - productStart, brandWarning,
