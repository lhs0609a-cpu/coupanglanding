...
/**
 * GET /api/megaload/products/thumbnail-jobs?batchId=...
 * 본인 잡 진행 현황 — 상태별 카운트 + (batchId 지정 시) 해당 배치 행 목록.
 */
  ...
  try {
    ...
    if (batchId) {
      const { data: rows } = await serviceClient
        .from('megaload_thumbnail_jobs')
        .select('id, product_code, label, status, result_url, error_message, completed_at')
        .eq('megaload_user_id', shUserId)
        .eq('batch_id', batchId)
