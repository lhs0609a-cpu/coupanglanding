...
export async function POST() {
  try {
    ...
    try {
      ...
      // 사용자 의도는 "지금 즉시 결제" — 보고서 없는 PT생도 함께 처리.
      let autoGenerated = 0;
      if (!unpaidReports || unpaidReports.length === 0) {
        const { data: targetUsers } = await serviceClient
          .from('pt_users')
          .select('id, profile_id, share_percentage, billing_excluded_until, status, is_test_account')
          .neq('status', 'terminated')
          .eq('is_test_account', false);

        for (const pt of targetUsers || []) {
          // 결제 제외 사용자 skip
          if (pt.billing_excluded_until && pt.billing_excluded_until >= todayDateStr) continue;
          ...
          if (!snap || !snap.total_sales || snap.total_sales <= 0) continue;
      ...
      if (!unpaidReports || unpaidReports.length === 0) {
        return NextResponse.json({
          ...
          message: autoGenerated === 0
            ? '청구 대상 리포트 없음 (보고서 자동 생성도 실패 — API 매출 데이터 없음)'
            : '보고서 자동 생성됐지만 청구 가능 상태 아님',
        });
      }
