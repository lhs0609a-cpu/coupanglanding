...
// Phase 1(즉시할인) 이후 Phase 2(다운로드) 진입 가능 한계.
// 다운로드 폴링이 최대 30초 + verify 한번 + 후속 verify까지 약 35초가 필요할 수 있어
// maxDuration(55s)에서 역산해 20초로 보수적 설정.
const TIMEOUT_SAFETY_MS = 20000;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    void logSystemError({ source: 'promotion/bulk-apply', error: message }).catch(() => {});
