...
import { fetchDownloadCoupons } from '@/lib/utils/coupang-api-client';
...
export async function GET() {
  try {
    ...
    const coupons = await fetchDownloadCoupons(credentials);

    return NextResponse.json({ data: coupons });
  } catch (err) {
    console.error('promotion download coupons error:', err);
    void logSystemError({ source: 'promotion/coupons/download', error: err }).catch(() => {});
