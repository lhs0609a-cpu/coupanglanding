...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sorted.length === 0) {
      return NextResponse.json({ error: '수집된 키워드가 없습니다.' }, { status: 404 });
    }

    // 4. 상위 100개에 대해 네이버 쇼핑 API로 상품수 수집
    const keywordNames = sorted.map((c) => c.keyword);
    const productCounts = await fetchShoppingCount(keywordNames, shopClientId, shopClientSecret);
