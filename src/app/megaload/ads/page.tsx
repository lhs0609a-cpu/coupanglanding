...
import {
  ...
  PowerOff, PlusCircle, ChevronDown, Settings2,
} from 'lucide-react';
...
const DEFAULT_RULE: AdRule = {
  ...
  // ★ 기본 기능 ① 매출 안 나는 광고 자동 OFF — 광고비 1,200원 쓰고 매출 0이면 끔
  auto_off_enabled: true,
  off_spend_threshold: 1200,
  off_max_sales: 0,
  auto_delete_enabled: false,
  delete_after_off_days: 7,
  // ★ 기본 기능 ② 신규 등록 상품 자동 광고
  auto_register_enabled: true,
  register_scope: 'all_new',
...
export default function AdsAutomationPage() {
  ...
  const save = useCallback(async () => {
    ...
    setSaving(true);
    try {
      // 마스터 enabled 는 두 기본 기능(OFF/자동등록) 또는 고급(ROAS) 중 하나라도 켜져 있으면 자동 ON.
      // (워커 루프가 enabled=true 인 규칙만 실행하므로, 기능만 켜도 자동으로 돌게 함)
      const effectiveEnabled = rule.auto_off_enabled || rule.auto_register_enabled || rule.bid_adjust_enabled;
      const payload = {
        ...
        mode: rule.mode,
        bid_adjust_enabled: rule.bid_adjust_enabled,
    ...
    <div className="max-w-3xl space-y-6 p-1">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            <b>매출이 안 나는 광고는 자동으로 끄고</b>, <b>새로 올린 상품은 자동으로 광고</b>를 돌립니다.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 flex items-start gap-2">
        ...
        <p className="text-xs text-amber-800 leading-relaxed">
          실제 켜기/끄기·등록은 <b>메가로드 도우미(PC 앱)</b>가 윙에 로그인된 상태에서 수행합니다(쿠팡 광고 API 부재).
          대상은 <b>자동운영·매출최적화(AI 자동광고)</b> 캠페인입니다. 도우미 “광고 자동화” 탭에서
          <b> 전체 점검(안전)</b>으로 확인한 뒤 <b>자동 실행</b>을 켜세요.
        </p>
      </div>

      {/* 규칙 설정 */}
      <div className="rounded-xl border border-gray-200 bg-white p-5 space-y-4">
        <h2 className="font-semibold text-gray-900">기본 자동화</h2>
        ...
        <div className="rounded-lg border border-gray-200 p-4 space-y-3">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm font-semibold text-gray-900">매출 안 나는 광고 자동 끄기</span>
          </label>
          <p className="text-xs text-gray-500 pl-6 -mt-1">
            최근 <b>{rule.lookback_days}일</b> 동안 광고비를 <b>{Number(rule.off_spend_threshold).toLocaleString()}원</b> 이상 썼는데
            광고 매출이 <b>{Number(rule.off_max_sales) === 0 ? '한 건도 없으면' : `${Number(rule.off_max_sales).toLocaleString()}원 이하이면`}</b> 그 광고를 자동으로 끕니다.
          </p>
          {rule.auto_off_enabled && (
            <div className="grid grid-cols-3 gap-3 pl-6">
              ...
              <Field label="광고 매출 ≤ (원)" hint="0 = 한 건도 없으면"><input type="number" value={rule.off_max_sales} onChange={num('off_max_sales')} className={inputCls} /></Field>
              <Field label="평가 기간 (일)" hint="최근 N일 기준"><input type="number" value={rule.lookback_days} onChange={num('lookback_days')} className={inputCls} /></Field>
            </div>
          )}
        </div>
        ...
        <div className="rounded-lg border border-gray-200 p-4 space-y-3">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm font-semibold text-gray-900">새로 올린 상품 자동 광고</span>
          </label>
          <p className="text-xs text-gray-500 pl-6 -mt-1">
            새로 등록된 상품을 <b>자동운영·매출최적화(AI 자동광고)</b>로 자동 등록합니다. 아래 상한으로 광고비 폭증을 막습니다.
          </p>
          {rule.auto_register_enabled && (
            <div className="space-y-3 pl-6">
              <div className="grid grid-cols-2 gap-2">
                {(['all_new', 'selected'] as const).map((sc) => (
                  <button key={sc} type="button" onClick={() => setRule((p) => ({ ...p, register_scope: sc }))}
                    ...
                    <b>{sc === 'all_new' ? '신규 등록 상품 전체' : '내가 고른 상품만'}</b>
                    <span className="block text-gray-500 mt-0.5">{sc === 'all_new' ? '새로 올라간 상품 자동 (상한 내)' : '상품관리에서 선택한 것만'}</span>
                  </button>
                ))}
              </div>
              <div className="flex items-start gap-1.5 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-2.5 py-2">
                ...
                <span><b>광고비 폭증 주의:</b> 상품당 일 예산 · 하루 등록 개수 · 전체 일예산 상한으로 비용을 통제합니다. 처음엔 작게 설정하세요.</span>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <Field label="상품당 일 예산 (원)"><input type="number" value={rule.register_daily_budget} onChange={num('register_daily_budget')} className={inputCls} /></Field>
                <Field label="하루 등록 상한 (개)" hint="폭증 방지"><input type="number" value={rule.register_max_per_day} onChange={num('register_max_per_day')} className={inputCls} /></Field>
                <Field label="전체 일예산 상한 (원)" hint="비우면 무제한">
        ...
        <button type="button" onClick={() => setShowAdvanced((v) => !v)}
          className="w-full flex items-center gap-2 text-xs font-medium text-gray-500 hover:text-gray-700 pt-1">
          <Settings2 className="w-3.5 h-3.5" />
          고급 설정 (실행 방식 · ROAS 입찰 조정 · 전환0 보호 · 자동 삭제)
        ...
        {showAdvanced && (
          <div className="space-y-5 border-t border-gray-100 pt-4">
            {/* 실행 방식 */}
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5">실행 방식</label>
              ...
              <p className="text-[11px] text-gray-400 mt-1.5">※ 기본은 <b>완전 자동</b>입니다. 처음엔 <b>드라이런</b>으로 며칠 확인 후 올리는 걸 권장.</p>
            </div>

            {/* ROAS 입찰 자동조정 */}
            <div className="rounded-lg border border-gray-200 p-3 space-y-3">
              <label className="inline-flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={rule.bid_adjust_enabled} onChange={(e) => setRule((p) => ({ ...p, bid_adjust_enabled: e.target.checked }))} className="w-4 h-4 accent-[#E31837]" />
                <span className="text-sm font-medium text-gray-700">ROAS 입찰 자동조정 사용 (목표 ROAS에 맞춰 입찰/예산 조정)</span>
              </label>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        <h2 className="font-semibold text-gray-900 mb-3">자동화 실행 이력 (끄기 · 등록 · 입찰)</h2>
        {changes.length === 0 ? (
          <p className="text-sm text-gray-400 py-6 text-center">아직 실행 내역이 없습니다. 도우미가 광고를 평가하면 여기에 표시됩니다.</p>
        ) : (
