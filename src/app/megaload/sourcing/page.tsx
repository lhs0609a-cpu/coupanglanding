...
import { SOURCING_PLATFORM_LABELS, SOURCING_PLATFORM_COLORS } from '@/lib/megaload/constants';
...
export default function SourcingPage() {
  ...
  const handleSearch = async () => {
    ...
    if (query.includes('aliexpress.com') || query.includes('1688.com')) {
      ...
      try {
        const res = await fetch('/api/megaload/sourcing/fetch', {
          method: 'POST',
      ...
      // 키워드 검색
      try {
        const res = await fetch('/api/megaload/sourcing/search', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const addToWishlist = async (product: SourcingResult) => {
    ...
    await supabase.from('sh_sourcing_wishlist').upsert({
      megaload_user_id: (shUser as Record<string, unknown>).id,
    ...
    }, { onConflict: 'megaload_user_id,platform,platform_product_id' });
  };
          ...
          {results.map((product) => (
            <div key={product.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition">
              ...
              <div className="p-3">
                ...
                <div className="flex gap-2">
                  <a
                    href={`/megaload/sourcing/register?productId=${product.id}&platform=${product.platform}`}
                  ...
                  <a
                    href={`/megaload/sourcing/simulator?cost=${product.price_cny}`}
