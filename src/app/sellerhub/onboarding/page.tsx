...
export default function OnboardingPage() {
  ...
  const testChannel = useCallback(async (channel: Channel, creds: Record<string, string>) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/sellerhub/channels/test', {
        method: 'POST',
  ...
  const saveCredentials = useCallback(async (channel: Channel, creds: Record<string, string>) => {
    await fetch('/api/sellerhub/channels/credentials', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ channel, credentials: creds }),
    });
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      ...
      setLoading(true);
      try {
        await fetch('/api/sellerhub/products/sync-coupang', { method: 'POST' });
      } catch {
        // 백그라운드 처리이므로 에러 무시
      }
      setLoading(false);
    ...
    if (step === 4) {
      ...
      router.push('/sellerhub/dashboard');
      return;
    }
        ...
        {step === 4 && (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500">
              SellerHub가 준비되었습니다. 대시보드에서 바로 시작하세요.
            </p>
          </div>
        )}
