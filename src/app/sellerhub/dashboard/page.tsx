...
import { CHANNELS, CHANNEL_SHORT_LABELS, CHANNEL_COLORS, CHANNEL_BG_COLORS, ORDER_STATUS_LABELS, ORDER_STATUS_COLORS } from '@/lib/sellerhub/constants';
...
export default function DashboardPage() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    const sellerhubUserId = (shUser as Record<string, unknown>).id as string;
    ...
    const [
      ...
      supabase.from('sh_orders').select('id', { count: 'exact', head: true })
        .eq('sellerhub_user_id', sellerhubUserId).eq('order_status', 'payment_done'),
      supabase.from('sh_orders').select('id', { count: 'exact', head: true })
        .eq('sellerhub_user_id', sellerhubUserId).eq('order_status', 'order_confirmed'),
      supabase.from('sh_orders').select('id', { count: 'exact', head: true })
        .eq('sellerhub_user_id', sellerhubUserId).eq('order_status', 'shipping_ready'),
      supabase.from('sh_cs_inquiries').select('id', { count: 'exact', head: true })
        .eq('sellerhub_user_id', sellerhubUserId).eq('status', 'pending'),
      supabase.from('sh_orders').select('*, sh_order_items(*)')
        .eq('sellerhub_user_id', sellerhubUserId)
  ...
  if (data.lowStockCount > 0) {
    alerts.push({ message: `재고 부족 상품 ${data.lowStockCount}개`, type: 'warning', link: '/sellerhub/inventory' });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Link href="/sellerhub/orders?status=payment_done" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
        ...
        <Link href="/sellerhub/orders?status=order_confirmed" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
          <div className="flex items-center gap-3">
        ...
        <Link href="/sellerhub/orders?status=shipping_ready" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
          <div className="flex items-center gap-3">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/sellerhub/orders" className="text-sm text-[#E31837] hover:underline flex items-center gap-1">
            ...
            <div key={order.id} className="bg-white rounded-xl border border-gray-200 p-3 hover:shadow-sm transition flex items-center gap-3">
              ...
              {order.order_status === 'payment_done' && (
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    await fetch(`/api/sellerhub/orders/${order.id}/confirm`, { method: 'POST' });
                    fetchDashboard();
                  }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Link href="/sellerhub/cs" className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
        ...
        <Link href="/sellerhub/inventory" className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
        ...
        <Link href="/sellerhub/sourcing" className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
        ...
        <Link href="/sellerhub/channels" className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition">
