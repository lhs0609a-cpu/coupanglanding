...
const SUBCATEGORY_ALIASES: Record<string, string> = {
  ...
  // ⚠️ 분유/유아식품 → '출산/유아동>분유' (DHA/ARA/유청단백/카제인/락토페린 풀)
  //    이전엔 '식품>건강식품'으로 redirect했으나 audit 16k에서 "엄마가 매일 쓰시는 프로틴",
  //    크릴오일/홍삼 등 영양제 어휘가 분유 본문에 누출되는 사고 발생 (2026-05-18).
  '출산/유아동>분유/유아식품':'출산/유아동>분유',
  '출산/유아동>분유/유아식품>분유':'출산/유아동>분유',
  '출산/유아동>분유/유아식품>이유식':'출산/유아동>유아식품',
  // 유아간식/음료는 '식품>음료' (V2 템플릿 존재) — 일반 가공음료 톤
  '출산/유아동>분유/유아식품>유아간식/음료':'식품>음료',
  '출산/유아동>분유/유아식품>유아건강식품':'출산/유아동>분유',
  // 유아국/반찬, 유아양념/식재료는 '식품' (generic) 사용 — 영양제 풀 누출 방지
  '출산/유아동>분유/유아식품>유아국/반찬':'식품',
  '출산/유아동>분유/유아식품>유아양념/식재료':'식품',
  '출산/유아동>분유/유아식품>일반분유':'출산/유아동>분유',
  '출산/유아동>분유/유아식품>특수분유':'출산/유아동>분유',
  '출산/유아동>분유/유아식품>산양분유':'출산/유아동>분유',
  '출산/유아동>분유/유아식품>영유아식':'출산/유아동>유아식품',
  // 수유/이유용품의 식품 관련 (분유 보관/포장 등)도 식품 톤
  // ⚠️ 이유/유아식기 → '출산/유아동>외출용품' (소재 톤). 이전엔 '유아식품'으로 redirect되어
  //    런치백/과즙망/스낵컵에 "DHA" 누출 (2026-05-18 audit). 식기는 식품 풀과 분리.
  '출산/유아동>수유/이유용품':'출산/유아동>분유','출산/유아동>이유/유아식기':'출산/유아동>외출용품',
  '출산/유아동>유아목욕/스킨케어':'출산/유아동>유아스킨케어','출산/유아동>유아물티슈/캡/홀더':'출산/유아동>기저귀',
  '출산/유아동>유아위생/건강/세제':'출산/유아동>기저귀',
  // ⚠️ 구강케어 / 욕실용품 / 야외활동용품 — 스킨케어 풀(히알루론산/판테놀) 비적합 → 기저귀 풀(생활용품 톤)로 분리
  //    audit 16k 발견: 치약/칫솔/고리타월에 "히알루론산", "판테놀" 누출 (2026-05-18).
  '출산/유아동>유아목욕/스킨케어>유아구강케어':'출산/유아동>기저귀',
  '출산/유아동>유아목욕/스킨케어>유아구강용품':'출산/유아동>기저귀',
  '출산/유아동>유아목욕/스킨케어>유아목욕/욕실용품':'출산/유아동>기저귀',
  '출산/유아동>유아목욕/스킨케어>야외활동용품':'출산/유아동>외출용품',
  // ⚠️ 놀이매트/안전용품 → '출산/유아동>외출용품' (알루미늄프레임/EPS폼/5점식벨트 풀)
  //    이전엔 '유아식품'으로 redirect되어 "두뇌발달 영양죽" 등 식품 어휘 누출 (2026-05-18 audit).
  '출산/유아동>놀이매트/안전용품':'출산/유아동>외출용품',
  '출산/유아동>외출용품':'출산/유아동>외출용품','출산/유아동>유아가구/인테리어':'출산/유아동>외출용품',
  // ⚠️ 유아동침구 / 임부용품 → '출산/유아동>외출용품' (소재 톤, 화장품 어휘 차단)
  //    이전엔 '유아스킨케어'로 redirect되어 베개/이불에 히알루론산/판테놀 누출 (2026-05-18 audit).
  '출산/유아동>유아동침구':'출산/유아동>외출용품','출산/유아동>임부용품':'출산/유아동>외출용품',
...
export function resolveVariables(
  ...
  //
  // ★ 가드: 일반 식품(신선식품/과일/채소/수산/축산/농산/음료/가공식품/조미료)에서는
  //   상품명에 우연히 들어간 단어("단백질 함량 표시" 등) 또는 displayName 오염으로
  //   헬스 디텍터가 활성화되어 망고 상세에 "프로틴/BCAA" 문구가 섞이는 사고 방지.
  const isNonSupplementFood = /^식품>(?:신선식품|과일|채소|수산|축산|농산|음료|가공식품|조미료|장류|발효식품|양념|식자재|간식|스낵|냉동식품)/.test(categoryPath);
  if (productName && /^(식품|건강식품|영양제|비타민)/.test(categoryPath) && !isNonSupplementFood) {
...
const L2_FORBIDDEN_TERMS: Record<string, string[]> = {
  ...
  '식품>신선식품': [
    ...
    '리뷰 마이닝','후기 마이닝','리뷰 평균 평점','리뷰 평균','리뷰 톤','부정 키워드','부정 표현','코멘트의 핵심','사용자 코멘트',
    '비교 영상','비교 콘텐츠','비교 글','비교 표',
    '동급 평균','동급 분포','동급에서 손꼽','동급에서 가장',
    '검색 비용','검색 비용 대비','결정 만족도','결정 시간','결정 망설임','검색 마무리','검색 끝','검색 끝에','옵션이에요','옵션입니다',
    // 카테고리/비즈니스 어휘 (audit Round 2 SEO weave에서 차단했지만 다른 path 누출)
    '카테고리에서','한 번쯤 짚어볼','짚어볼 만한','자주 비교되는','자주 비교',
    // 활용 어색 표현 (비문)
    '활용을 추천','활용을 추천드립니다','활용을 추천해요','활용해 드세요','활용 드시',
    // 가공식품 인증 (생과일 대상 아님)
    'HACCP','GMP','검증으로 안심',
    // 영양제 어휘
    '비타민 엄선','엄선한 비타민',
...
const V2_GLOBAL_FRAGMENTS: Record<string, string[]> = {
  hook: [
    ...
    '주말에 시간 여유 생기면 꼭 챙기게 되는 {product}이에요.','월요일 아침에 챙겨두면 한 주가 가벼워지는 {product}이에요.',
    '비 오는 날 더 생각나는 게 {product}이에요.','계절이 바뀔 때마다 다시 찾게 되는 {product}이에요.',
    '집들이 선물 고민하다가 결국 {product}으로 정했어요.','부모님께 드리고 싶은 게 뭘까 한참 고민하다 {product} 골랐어요.',
    '동료한테 받은 추천 중에 가장 만족스러웠던 게 {product}이에요.','커뮤니티 후기에 자주 언급돼서 한번 사봤는데, 잘 샀다는 생각이 드는 {product}이에요.',
    '안 사고는 못 배긴다는 표현이 어울리는 {product}이에요.','일단 한 번 들이면 다시는 안 사고 못 배겨요. {product} 얘기예요.',
    '괜찮은 거 골랐다 싶은 순간이 있는데, {product}이 딱 그런 케이스예요.','쓸데없이 비싼 거 사면 후회만 남는데, {product}은 그 반대예요.',
    '쟁여두고 싶은 마음이 드는 {product}이에요.','다음에 또 사야지 싶은 게 {product}이에요.',
    '여행 갈 때 짐에 꼭 챙겨가는 {product}이에요.','선물 받았는데 직접 사고 싶어진 {product}, 결국 또 샀어요.',
    '한 번 사봤다가 단골 된 {product}입니다.','집에 항상 떨어지지 않게 두는 {product}이에요.',
    '바쁠 때일수록 더 빛나는 {product}이에요.','쉬는 날 한 번 써보면 진가를 아는 {product}이에요.',
    '특별한 날에도 평범한 날에도 어울리는 {product}이에요.','매일 쓰는 거라 더 신중하게 골랐던 {product}, 후회 없어요.',
  ],
  problem: [
  ...
  usage_guide: [
    ...
    // ✨ Phase 11: 사용가이드 풀 확장 (length 부족 해결)
    '처음 며칠은 천천히 적응하는 마음으로 사용하시면 가장 자연스러워요.',
    '사용 환경에 따라 약간씩 다를 수 있지만 큰 차이는 없으니 부담 없이 시작하세요.',
    '간단한 사용법인데도 후기에서는 활용도가 높다는 평이 많아요.',
    '익숙해지면 별다른 신경 없이 자연스럽게 일상에 녹아들어요.',
    '사용 빈도를 조금씩 늘려가시면 만족도도 함께 올라가요.',
    '특별한 기술 없이도 누구나 충분히 활용하실 수 있어요.',
    '평소 쓰는 흐름 안에 자연스럽게 끼워두시면 부담이 적어요.',
    '꾸준한 사용이 가장 좋은 활용법이라는 평이 많아요.',
    '한두 번 써보시면 자기에게 맞는 사용 방식이 자연스럽게 잡혀요.',
    '복잡한 안내서 없이도 직관적으로 다룰 수 있어요.',
  ],
  social_proof: [
    ...
    // ✨ Phase 11: social_proof 풀 확장
    '”고민 없이 다시 산다”는 후기가 자주 보이는 종류의 제품이에요.',
    '추천글 작성자 본인이 다시 구매한 흔적이 자주 발견돼요.',
    '구매 후 시간이 지나도 만족도가 떨어지지 않는다는 평이 많아요.',
    '”잘 산 제품 추천해달라”는 글에 자주 답글로 등장해요.',
    '단골 비율이 평균을 상회하는 점이 가장 큰 강점이에요.',
    '여러 옵션을 둘러본 분들이 결국 정착하는 종류의 제품이에요.',
    '후기에서 “기대 이상”이라는 표현이 유독 자주 등장해요.',
    '실제 사용자의 자발적 추천이 새 사용자 유입의 가장 큰 동력이에요.',
    '리뷰 분포가 한쪽에 치우치지 않고 고르게 좋은 평가를 받아요.',
    '구매 후 시간이 지난 후의 후기일수록 만족도가 높아지는 경향이 있어요.',
  ],
  feature_detail: [
    ...
    // ✨ Phase 11: length 부족 해결을 위한 풀 확장 (가구·매트리스·세부 식품 등 작은 풀 보강)
    '필요한 핵심 기능만 깔끔하게 모아둔 구성이라 만족도가 높은 편이에요.',
    '오래 사용해도 첫 인상의 만족감이 그대로 유지된다는 후기가 많아요.',
    '눈에 보이지 않는 부분까지 꼼꼼하게 다듬은 흔적이 손끝에서 느껴져요.',
    '사용 흐름에 자연스럽게 녹아드는 구성이라 적응 시간이 거의 필요 없어요.',
    '꼭 필요한 곳에 정확히 힘을 준 균형이 인상적이에요.',
    '실사용에서 손이 가장 자주 닿는 부분의 완성도가 높아요.',
    '평범해 보이지만 작은 디테일이 매일의 만족감을 만들어요.',
    '간결한 구성이라 처음 쓰시는 분도 어렵지 않게 익숙해지실 거예요.',
    '쓸수록 점수가 올라가는 타입이라 시간이 지날수록 만족도가 깊어져요.',
    '복잡한 기능보다 매일 쓰는 핵심에 집중해 부담이 적어요.',
    '실속파에게 특히 잘 맞는 균형감이 가장 큰 매력이에요.',
    '겉모습보다 사용감의 안정성에 무게가 실린 제품이에요.',
    '필요한 만큼만 정확히 담아낸 깔끔한 구성이 매력이에요.',
    '쓰면서 작은 불편이 거의 없는 점이 가장 큰 장점이에요.',
    '한 번 자리 잡으면 자연스럽게 일상이 된다는 평이 많아요.',
    '"있어서 좋다"보다 "없으면 허전하다"는 후기가 많이 달리는 타입이에요.',
    '광고나 화려한 마케팅 없이 사용자가 먼저 알아보는 종류의 제품이에요.',
    '오랫동안 손이 가는 제품엔 이유가 있다는 걸 보여줘요.',
    '겉으로 드러나지 않아도 매일의 만족도에 분명한 차이를 만들어요.',
    '비슷해 보이는 제품들 사이에서 결국 다시 손이 가는 타입이에요.',
  ],
  urgency: [
...
export function mergeVariables(
  ...
  if (categoryPath && isFoodCategory(categoryPath)) {
    ...
    // 일반 식품(신선/과일/채소/수산/축산/농산/음료/가공)에는 헬스보충제 어휘 추가 차단.
    // 단, 카테고리가 명시적으로 건강식품/영양제/단백질 보충제면 유지.
    const isHealthSupplement = /건강식품|영양제|보충제|프로틴파우더|복합 프로틴/.test(categoryPath);
    if (!isHealthSupplement) {
      ['프로틴', '단백질', 'BCAA', 'WPC', 'WPI', '크레아틴', '카제인', '게이너',
       '식이섬유', 'HCA', 'CLA', '코엔자임Q10', '유비퀴놀', '밀크씨슬', '실리마린',
       '루테인', '지아잔틴', '글루코사민', '콘드로이친', 'MSM', '쏘팔메토',
       '오메가3', 'EPA', 'DHA', '홍삼', '진세노사이드', '프로폴리스',
       '비오틴', '판토텐산', '판테놀', '아르간오일',
      ].forEach((t) => FOOD_BANNED_INGREDIENT.add(t));
    }
