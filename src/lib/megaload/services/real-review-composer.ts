...
const SUBCATEGORY_ALIASES: Record<string, string> = {
  ...
  // 식기/런치백 → 외출용품 (소재 톤, DHA 누출 차단)
  '출산/유아동>이유/유아식기':       '출산/유아동>외출용품',
  '출산/유아동>유아목욕/스킨케어':   '출산/유아동>유아스킨케어',
  // 구강케어/욕실용품/야외활동 → 스킨케어 풀 부적합 (audit 16k 발견 — 치약에 히알루론산 누출)
  ...
  // 안전용품 / 침구 / 임부용품 → 외출용품(소재 풀). 이전엔 유아식품/스킨케어로 매핑되어
  // 머리보호대에 DHA, 베개에 히알루론산이 누출 (2026-05-18 audit 발견).
  '출산/유아동>놀이매트/안전용품':   '출산/유아동>외출용품',
  '출산/유아동>외출용품':            '출산/유아동>외출용품',
  '출산/유아동>유아가구/인테리어':   '출산/유아동>외출용품',
  '출산/유아동>유아동침구':          '출산/유아동>외출용품',
  '출산/유아동>임부용품':            '출산/유아동>외출용품',
...
function resolveVariablePoolCore(
  ...
  if (catKey === '식품') {
    if (/비타민|오메가|유산균|프로바이오|루테인|밀크씨슬|홍삼|캡슐|정제|영양제|글루코사민|콜라겐|건강기능|건강식품|비오틴/.test(productName) ||
      ...
      } else if (/블루베리|빌베리|아로니아|크랜베리|복분자|오디|블랙커런트|베리\b|안토시아닌|항산화/i.test(pn) || /블루베리|빌베리|아로니아|크랜베리|복분자/.test(categoryPath)) {
        // 베리류 (건강식품 카테고리) — 안토시아닌/항산화 중심. 영양제 일반 성분(루테인/오메가3) 차단.
        healthVars['효과1'] = ['항산화','눈건강','피부건강','노화방지','혈관건강','면역력','피로회복','시력관리'];
        healthVars['효과2'] = ['항산화력','눈피로회복','피부윤기','노화억제','면역증진','활력개선'];
        healthVars['성분'] = ['안토시아닌','폴리페놀','프로안토시아니딘','레스베라트롤','케르세틴','비타민C','비타민E','베리추출물','블루베리추출물','빌베리추출물'];
        healthVars['카테고리'] = ['블루베리','베리류','항산화영양제','건강식품','눈영양제'];
      }
