...
import { pickAndScanFolder, uploadScannedImages } from '@/lib/sellerhub/services/client-folder-scanner';
import { validateProductLocal } from '@/lib/sellerhub/services/product-validator';
import type {
...
export function useBulkRegisterActions() {
    ...
    try {
      ...
      for (const fp of serverPaths) {
        ...
        const editableProducts: EditableProduct[] = (data.products as PreviewProduct[]).map((p) => ({
          ...p, uid: `${p.folderPath}::${p.productCode}`, editedName: p.name, editedBrand: p.brand,
  ...
  return {
    ...
    handleRegister, togglePause, handleReset,
  };
}
