...
import { detectAllWeights, isAgriProduct, resolveAgriWeight, rewriteDisplayNameForPickedWeight } from './option-candidates';
...
interface CoupangFieldsSectionProps {
  ...
  /** 카테고리 속성/옵션 메타 — 필수 구매옵션 영역 렌더에 사용 */
...
/* ─── 중량/용량 옵션값 인라인 편집 ───
   상품명에서 추출된 중량을 "항상" 편집 가능하게 노출(옵션/아이템 섹션).
   기존엔 농산물이거나 중량 후보 2개+일 때만 떠서, 100g 같은 단일 중량 일반상품은
   수정 컨트롤이 아예 없었음 → 사용자가 옵션 수정 불가. draft 로컬상태로 편집 중 깜빡임 방지. */
function WeightOptionField({
  product,
  onUpdate,
}: {
  product: EditableProduct;
  ...
  const chips = Array.from(new Set([
    ...detectAllWeights(product.editedDisplayProductName || product.name),
    ...detectAllWeights(product.name),
  ]));
  const picked = product.editedAgriWeight ?? '';
  const resolved = resolveAgriWeight(product.editedDisplayProductName, product.name, picked);
  const effective = picked || resolved || '';
  const [draft, setDraft] = useState(effective);
  useEffect(() => { setDraft(effective); }, [effective]);

  // 농산물이거나 상품명에 중량이 1개라도 있으면 항상 편집 노출
  if (!isAgri && chips.length < 1) return null;

  const ambiguousMulti = chips.length >= 2 && !picked;
  const unresolved = (isAgri && !resolved) || ambiguousMulti;

  const commit = (w: string) => {
    const v = w.trim();
    if (!v || v === effective) return;
    onUpdate(product.uid, 'editedAgriWeight', v);
    const base = product.editedDisplayProductName ?? '';
    if (base) {
      const rewritten = rewriteDisplayNameForPickedWeight(base, v);
      if (rewritten !== base) {
        onUpdate(product.uid, 'editedDisplayProductName', rewritten);
        onUpdate(product.uid, 'autoGeneratedDisplayName', rewritten);
      }
    }
  };

  return (
    <div className={unresolved ? 'border-l-2 border-l-red-400 pl-3' : ''}>
      {isAgri
        ? <RequiredLabel empty={unresolved}>중량/용량 (옵션값) (필수)</RequiredLabel>
        : <OptionalLabel>중량/용량 (옵션값)</OptionalLabel>}
      <input
        type="text"
        value={draft}
        onChange={(e) => setDraft(e.target.value)}
      ...
      <p className={`text-[10px] mt-0.5 ${unresolved ? 'text-red-500' : 'text-gray-400'}`}>
        {isAgri && !resolved
          ? '⚠ 중량 미확정 — 쿠팡윙 "옵션 용량 오류" 방지를 위해 실제 중량을 입력/선택하세요 (미입력 시 등록 차단).'
          : ambiguousMulti
            ? `원본에 여러 중량(${chips.join(', ')})이 있습니다. 등록할 중량을 고르세요.`
            : effective
              ? `적용 중량: ${effective} — 수정하면 노출상품명 끝 중량도 자동 반영`
              : '예: 100g / 500g / 5kg'}
      </p>
    </div>
  );
