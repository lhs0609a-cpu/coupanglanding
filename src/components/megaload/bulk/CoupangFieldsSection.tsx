            ...
            <div className={unresolved ? 'border-l-2 border-l-amber-400 pl-3' : ''}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  onUpdate(product.uid, 'editedAgriWeight', next);
                  // 옵션값↔노출상품명 미스매치 방지: 선택 즉시 노출상품명 꼬리 spec 재합성.
                  // autoGeneratedDisplayName 도 함께 갱신해서 "사용자 편집" 으로 오인되지 않게 한다
                  // (그래야 등록 시 displayProductNameOverride 가 비워져 서버 sync 가 정상 동작).
                  const base = product.editedDisplayProductName ?? '';
                  if (base) {
                    const rewritten = rewriteDisplayNameForPickedWeight(base, next);
                    if (rewritten !== base) {
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">
                ...
                미선택 시 &quot;{REFER_DETAIL}&quot;로 등록됩니다(노출상품명 꼬리 자동 정리).
              </p>
            </div>
          );
        })()}
