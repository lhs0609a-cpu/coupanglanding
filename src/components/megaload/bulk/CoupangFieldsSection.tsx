  ...
  return (
    <div className="space-y-2.5">
      ...
      >
        ...
        {/* 농산물 중량 — 신선식품/농산물은 항상 입력/선택 (쿠팡윙 "옵션 용량 오류" 방지).
            그 외 카테고리는 원본에 중량 후보가 2개 이상일 때만 노출(기존 동작 유지). */}
        {(() => {
          const isAgri = isAgriProduct(product.editedCategoryName);
          ...
          if (!isAgri && chips.length < 2) return null;
          ...
          // 선택 즉시 노출상품명 꼬리 spec 재합성 (옵션값↔제목 미스매치 방지).
          // autoGeneratedDisplayName 도 같이 갱신해 "사용자 편집"으로 오인되지 않게 한다.
          const setWeight = (w: string) => {
            onUpdate(product.uid, 'editedAgriWeight', w);
            const base = product.editedDisplayProductName ?? '';
            if (base) {
              const rewritten = rewriteDisplayNameForPickedWeight(base, w);
              if (rewritten !== base) {
          ...
          return (
            <div className={unresolved ? 'border-l-2 border-l-red-400 pl-3' : ''}>
              <RequiredLabel empty={unresolved}>농산물 중량{isAgri ? ' (필수)' : ''}</RequiredLabel>
              <input
                type="text"
                value={picked}
                onChange={(e) => onUpdate(product.uid, 'editedAgriWeight', e.target.value)}
                onBlur={(e) => { const v = e.target.value.trim(); if (v) setWeight(v); }}
                className={inputRequired(unresolved)}
                placeholder="예: 5kg, 500g"
              />
              {chips.length > 0 && (
