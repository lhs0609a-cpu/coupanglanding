...
export function useBulkRegisterActions() {
  ...
  // ─── 필수 속성 자동기입 ───────────────────────────────────────────
  // attributeMeta(16k 카테고리 캐시)가 있는 모든 상품의 editedAttributeValues 를 서버
  //   buildAttributes 와 동일 규칙으로 미리 채운다 → "필수 미입력" 거짓 차단 해소(전 카테고리).
  //   중량/용량은 상품명에서 추출되면 채우고 못 뽑으면 비워 둠(안전우선, 경고 유지).
  ...
  useEffect(() => {
    ...
    for (const p of products) {
      ...
      if (!meta?.attributeMeta?.length) continue;
      ...
      setProducts(prev => {
        ...
        for (let i = 0; i < targets.length; i++) {
          ...
          if (globalIdx >= 0) {
            ...
            const finalDpn = generatedTrim || target.name.slice(0, 100);
            updated[globalIdx] = {
              ...updated[globalIdx],
              editedDisplayProductName: finalDpn,
              // 자동값과 동일한지 등록 시점에 비교해 "사용자 편집 여부" 를 판정한다.
              autoGeneratedDisplayName: finalDpn,
              editedSellerProductName: updated[globalIdx].editedName,
            };
          }
        }
  ...
  useEffect(() => {
    if (autoMatchingProgress === null && autoMatchStats && !pipelineRan && step === 2) {
      ...
      (async () => {
        ...
        {
          const { filterAndScoreMainImages, clearHistogramCache, clearAnalysisCache } = await import('@/lib/megaload/services/image-quality-scorer');
          ...
          // 자동 제외는 "명백한 쓰레기"(빈 이미지·텍스트 배너·글자판·홍보배너)만 대상으로 한다.
          // 점수 저하/색상 outlier 기반 제외는 정상 사진을 잘못 배제했던 이력이 있어 폐기 —
          // 정상 사진은 전부 등록하고, 대표는 점수 최고 사진 자동 승격으로만 결정한다.
            ...
            if (p.scannedMainImages && p.scannedMainImages.length > 0) {
              ...
              if (validEntries.length > 1) {
                try {
                  ...
                  // ─── 자동 제외 — "명백한 쓰레기" 이미지만 ───
                  // 빈 이미지 / 텍스트 배너 / 글자판 / 홍보배너만 등록에서 제외한다.
                  // ★ low_score / color_outlier / unrelated_to_main 은 더 이상 제외하지 않음:
                  //   정상 사진은 전부 등록하고, 대표(1번)는 아래 setProducts에서
                  //   "점수 최고 사진"으로 자동 승격하는 방식으로만 결정한다.
                  // ★ 모델얼굴(skin_tone)·컬러배경(colored_background)도 제외 대상 아님 —
                  //   리스트엔 남기고 대표 자리만 점수로 밀려나게 한다.
                  const reasonMap = autoExcludeMaps[idx];
                  const GARBAGE_REASONS = new Set<string>([
                    'empty_image', 'text_banner', 'text_only_image', 'promotional_image',
                  ]);
                  for (const s of scores) {
                    const origIdx = validEntries[s.index].origIdx;
                    const r = s.score.hardFilterReason;
                    if (r && GARBAGE_REASONS.has(r)) {
          ...
          setProducts(prev => prev.map((p, i) => {
            ...
            // ─── 대표(index 0) 자동 승격 (품질 × 지재권 안전도) ───
            // 폴더 첫 파일 고정을 폐기하고, "품질 × 안전도 최고 사진"을 대표(1번)로 자동 승격한다.
            // ★ 사진은 하나도 삭제하지 않음 — 순서만 재배치 (전자동: 사람이 폴더 정렬 안 해도 됨).
            //   · 자동 제외(쓰레기) 표시 사진 → 맨 뒤로 가라앉힘 (등록 시 어차피 필터됨)
            //   · rank = overall × safetyFactor (워터마크·모델얼굴·문구·인포그래픽이면 안전도↓ → 대표에서 밀림)
            //   · safetyFactor = 0.4 + 0.6×(safety/100) — 위험 사진도 완전 0은 아니라, 다 위험하면 그중 최선이 대표
            const taggedScored = scores.map(s => {
              const safety = s.score.safety ?? 100;
            ...
            taggedScored.sort((a, b) => {
              const ax = a.img.autoExcludeReason ? 1 : 0;
              const bx = b.img.autoExcludeReason ? 1 : 0;
              if (ax !== bx) return ax - bx;     // 쓰레기 표시는 뒤로
              return b.rank - a.rank;             // 품질 × 안전도 결합 점수 내림차순
            });
            const orderedImages = taggedScored.map(t => t.img);
            // 스코어가 없는 이미지(objectUrl 없음 등)는 원래 순서로 뒤에 보존 (삭제 방지)
            const scoredIdxSet = new Set(scores.map(s => s.index));
            const unscoredImages = p.scannedMainImages
              .map((img, j) => ({ img, j }))
    ...
    try {
      ...
      const batchProducts = selectedProds.map(p => {
        ...
        return {
          ...
          categoryConfidence: p.categoryConfidence,
          // 자동 생성값 그대로면 override 를 비워 서버에서 옵션값과 꼬리 spec 을 동기화.
          // 사용자가 직접 편집한 경우에만 override 로 핀(pin) 해서 보존.
          displayProductNameOverride: (p.editedDisplayProductName && p.editedDisplayProductName !== p.autoGeneratedDisplayName)
            ? p.editedDisplayProductName
            : undefined,
      ...
      const res = await fetch('/api/megaload/products/bulk-register/canary', {
        ...
        body: JSON.stringify({
          product: {
            ...
            // 자동 생성값 그대로면 override 를 비워 서버에서 옵션값과 꼬리 spec 을 동기화.
            displayProductNameOverride: (product.editedDisplayProductName && product.editedDisplayProductName !== product.autoGeneratedDisplayName)
              ? product.editedDisplayProductName
              : undefined,
    ...
    try {
      ...
      for (let cstart = 0; cstart < batches.length; cstart += CHUNK_PARALLEL) {
        ...
        for (const p of batch) {
          ...
          if (p.editedDisplayProductName) {
            // aiDisplayName 은 항상 전달(노출명 컨텍스트 보존), override 는 사용자가 자동값을 손본 경우에만 핀.
            product.aiDisplayName = p.editedDisplayProductName;
            if (p.editedDisplayProductName !== p.autoGeneratedDisplayName) {
              product.displayProductNameOverride = p.editedDisplayProductName;
            }
          }
