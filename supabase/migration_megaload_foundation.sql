-- ============================================================
-- Megaload 멀티채널 이커머스 자동화 플랫폼 — Foundation DDL
-- ============================================================

-- 1. 계정/인증
CREATE TABLE IF NOT EXISTS megaload_users (
...
CREATE TABLE IF NOT EXISTS megaload_sub_accounts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS sh_daily_sales_stats (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(megaload_user_id, stat_date, channel)
);
...
CREATE TABLE IF NOT EXISTS sh_credits (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  megaload_user_id UUID NOT NULL REFERENCES megaload_users(id) ON DELETE CASCADE UNIQUE,
  ai_credits INTEGER NOT NULL DEFAULT 500,
...
-- 9. 인덱스
CREATE INDEX IF NOT EXISTS idx_sh_products_user ON sh_products(megaload_user_id);
...
CREATE INDEX IF NOT EXISTS idx_sh_orders_user_status ON sh_orders(megaload_user_id, order_status);
...
CREATE INDEX IF NOT EXISTS idx_sh_cs_inquiries_user ON sh_cs_inquiries(megaload_user_id, status);
CREATE INDEX IF NOT EXISTS idx_sh_settlements_user ON sh_settlements(megaload_user_id, settlement_date);
CREATE INDEX IF NOT EXISTS idx_sh_api_logs_channel ON sh_api_call_logs(channel, called_at);
CREATE INDEX IF NOT EXISTS idx_sh_notifications_user ON sh_notifications(megaload_user_id, is_read);
CREATE INDEX IF NOT EXISTS idx_sh_sync_jobs_user ON sh_sync_jobs(megaload_user_id, status);

-- 10. RLS 기본 정책 (필요 시 활성화)
-- ALTER TABLE megaload_users ENABLE ROW LEVEL SECURITY;
