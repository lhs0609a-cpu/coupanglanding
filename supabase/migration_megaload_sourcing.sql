...
CREATE TABLE IF NOT EXISTS sh_sourcing_sources (
  ...
  UNIQUE(megaload_user_id, platform)
);
...
CREATE TABLE IF NOT EXISTS sh_sourcing_wishlist (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(megaload_user_id, platform, platform_product_id)
);
...
-- 10. 인덱스
CREATE INDEX IF NOT EXISTS idx_sh_sourcing_products_user ON sh_sourcing_products(megaload_user_id);
CREATE INDEX IF NOT EXISTS idx_sh_sourcing_products_platform ON sh_sourcing_products(platform, platform_product_id);
CREATE INDEX IF NOT EXISTS idx_sh_sourcing_orders_user ON sh_sourcing_orders(megaload_user_id, status);
...
CREATE INDEX IF NOT EXISTS idx_sh_wishlist_user ON sh_sourcing_wishlist(megaload_user_id);
