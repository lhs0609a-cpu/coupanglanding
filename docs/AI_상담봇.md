...
```
두 세트를 돌린다. **홀드아웃 숫자만 믿어야 한다.**

| 세트 | 성격 | 1위 정확도 | 3위 내 |
|---|---|---|---|
| 튜닝 세트 (67문항) | 문서를 만들며 같이 손봄 → 부풀려짐 | 100% | 100% |
| **홀드아웃 (30문항)** | **튜닝 안 함. 진짜 실력** | **76.7%** | **93.3%** |

측정 이력 — 왜 홀드아웃이 필요한지가 여기 다 있다.
1. 최초: 50문항 1위 **48%**
2. 검색 스코어링 개선(구절 매칭·화면문서 감점·priority 강화): **76%**
3. 질문별 태그 보강: 튜닝 세트 **100%** ← 문제집에 답 적어 넣은 것
4. 홀드아웃으로 재측정: **43.3%** ← 3번이 허수였음이 드러남
5. 용어 사전(glossary.ts) 도입: 홀드아웃 **76.7%**

**교훈: 홀드아웃 점수가 떨어지면 태그로 메우지 말 것.** 사전이나 문서를 고쳐야 한다.
스크립트가 실패 시 그렇게 경고하고, CI 판정도 홀드아웃으로만 한다(1위 70% / 3위내 88%).

### 왜 어휘 검색만으로는 한계가 있나
사용자는 문서와 다른 말을 쓴다. "똑같은 걸 두 번 올렸다"(→중복),
"키 넣었는데 연결이 안 돼요"(→인증), "보긴 보는데 안 사요"(→전환율).
`glossary.ts` 가 구어↔문서말을 이어주지만 사전에 없는 말은 여전히 못 넘는다.

**LLM 키가 켜지면 이 한계가 크게 완화된다.** 모델이 `search_kb` 를 부를 때
사용자 문장을 그대로 넣지 않고 **자기가 다시 쓴 검색어**를 넣기 때문이다.
즉 76.7% 는 바닥이지 천장이 아니다. 지금은 키가 꺼져 있어 사용자 문장이
그대로 검색기로 가므로, 이 숫자가 곧 사용자 체감이다.

---

## 8. 1만 명이 쓴다면 — 점검한 것과 남은 것

### 고친 것
| 문제 | 조치 |
|---|---|
| `recentFailures` 에 사용자 필터 누락 → **전체 사용자 실패 건수**를 모두에게 보여줌 | sh_products 조인으로 본인 것만 |
| `registeredCount` 가 상품 행 20,000개를 받아 JS 로 카운트 (상담 1회에 수 MB) | `count: 'exact', head: true` |
| 레이트 리밋 없음 → 비로그인 호출로 LLM 비용 무방비 | DB RPC 원자적 카운터. 로그인 10/분·150/일, 비로그인 5/분·30/일 |
| 429 를 위젯이 "연결 오류"로 뭉갬 | 본문 문구 그대로 노출 |

레이트 리밋 키는 **profile_id / IP** 다. `anon_key` 는 클라이언트가 만드는 값이라
얼마든지 새로 발급할 수 있어 제한 키로 쓸 수 없다.
RPC 가 없거나 실패하면 **통과**시킨다 — 리밋 장애로 상담이 막히는 게 더 나쁘다.

### 상담 1회당 DB 부하
- 매 요청: 레이트 리밋 RPC 1 + 사용자 조회 3 + 대화/메시지 2~3
- **새 대화 첫 턴에만**: 계정 진단 약 12 쿼리 (전부 count/단건)
- KB: 정적 341건은 인스턴스당 1회 조립, DB KB 는 5분 TTL

### 남은 것 (의도적으로 안 한 것)
- **대화 보존 정책 없음** — 무한히 쌓인다. 1만 명이면 정리 크론이 필요하다.
  (예: 90일 지난 미평가 대화 삭제)
- **임베딩 검색 없음** — 사전으로 못 넘는 말바꿈은 남는다. 서버 LLM 키가 생기면
  그때 검토할 일이다.
- **LLM 키 미설정** — 가장 큰 품질 제약. 이게 켜져야 대화형 진단과 질의 재작성이 산다.
