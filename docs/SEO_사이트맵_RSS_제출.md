...
megaload.co.kr 의 색인용 피드는 전부 코드에서 생성된다. 별도 파일을 public 에 올리지 않는다.

| 주소 | 생성 파일 | 내용 |
| --- | --- | --- |
| https://megaload.co.kr/sitemap.xml | `src/app/sitemap.ts` | 공개 URL 전체(홈·PT·프로그램·가이드·채널가이드·약관 등) |
| https://megaload.co.kr/rss.xml | `src/app/rss.xml/route.ts` | 콘텐츠 문서만(가이드 아티클 8 + 채널 입점 가이드 7 + 오픈마켓 비교 1) |
| https://megaload.co.kr/robots.txt | `src/app/robots.ts` | 크롤러 규칙 + 위 두 주소 고지 |
| https://megaload.co.kr/llms.txt | `src/app/llms.txt/route.ts` | AI 검색 크롤러용 요약 |
...
1. **사이트 소유확인** — 이미 완료 상태. 메타태그 방식이고 토큰은 `src/app/layout.tsx` 의
...
**검색 반영 현황**과 `site:megaload.co.kr` 검색으로 확인한다.
...
https://www.bing.com/webmasters — Sitemaps 에 `sitemap.xml`, `rss.xml` 둘 다 등록.
...
```bash
curl -s https://megaload.co.kr/rss.xml | head -20        # 200 + application/rss+xml
curl -s https://megaload.co.kr/sitemap.xml | grep -c "<loc>"
curl -s https://megaload.co.kr/robots.txt | grep Sitemap  # 두 줄 나와야 정상
```
